...
class MainWindow(QWidget):
    ...
    def _build_table(self):
        ...
        # Chon nhieu dong bang Shift/Cmd de copy ca cum mot lan
        ...
        # Cmd+C tren bang. QTableView KHONG tu copy — khong gan phim tat thi
        # bam Cmd+C xong khong co gi vao clipboard.
        # WidgetShortcut: chi an khi con tro dang o trong bang, khong cuop
        # Cmd+C cua o dan serial hay cua khung log.
        copy_shortcut = QShortcut(QKeySequence(QKeySequence.Copy), self.table)
    ...
    def _visible_rows(self, rows=None):
        """Đọc bảng ra list-of-list, theo ĐÚNG thứ tự đang nhìn thấy.

        Đi qua proxy chứ không qua model gốc: người dùng vừa lọc và vừa sắp
        xếp xong thì cái họ muốn copy là cái trên màn hình, không phải thứ tự
        lúc chạy.
        """
        if rows is None:
        ...
        for row in rows:
            bang.append([
                self.proxy.index(row, cot).data() or ""
                for cot in (COL_SERIAL, COL_DATE, COL_STATUS)
            ])
        return bang

    def _put_on_clipboard(self, text, so_dong):
        if not text:
            self._append_log("📋 Không có dòng nào để copy")
            return
        QGuiApplication.clipboard().setText(text)
        self._append_log(f"📋 Đã copy {so_dong} dòng — dán thẳng vào Excel được")

    def _copy_selection(self):
        """Cmd+C: copy các dòng đang chọn. Chưa chọn gì thì copy cả bảng."""
        chon = self.table.selectionModel().selectedRows()
        if not chon:
            self._copy_all()
            return
        so_dong = sorted(index.row() for index in chon)
        bang = self._visible_rows(so_dong)
        self._put_on_clipboard(rows_to_clipboard_text(bang), len(bang))

    def _copy_all(self):
        bang = self._visible_rows()
        self._put_on_clipboard(
            rows_to_clipboard_text(bang, with_header=True), len(bang))
    ...
    def _table_menu(self, point):
        ...
        # Chuot phai vao mot dong ngoai vung dang chon thi chon no truoc, de
        # "Copy dong dang chon" khong copy nham dong o cho khac.
        dang_chon = {i.row() for i in self.table.selectionModel().selectedRows()}
        if index.row() not in dang_chon:
        ...
        da_chon = len(self.table.selectionModel().selectedRows())
        copy_rows = QAction(
            f"Copy {da_chon} dòng đang chọn" if da_chon > 1 else "Copy cả dòng này",
            self)
        copy_rows.setShortcut(QKeySequence(QKeySequence.Copy))
        copy_rows.triggered.connect(self._copy_selection)
        menu.addAction(copy_rows)

        copy_all = QAction(
            f"Copy cả bảng ({self.proxy.rowCount()} dòng, kèm tiêu đề)", self)
    ...
    def _reveal_in_finder(self, path):
        """Hiện file trong Finder. True nếu chắc chắn đã làm được."""
        try:
            xong = subprocess.run(["open", "-R", str(path)],
    ...
    def _open_results(self):
        """Mở file CSV kết quả bằng app mặc định (Numbers / Excel).

        Bản cũ chỉ gọi `open -R`, tức là HIỆN file trong Finder chứ không mở
        nó — mà Finder lại hay không nhảy lên trước app, nên bấm nút xong
        không thấy gì xảy ra. Thêm nữa `check=False` nuốt sạch mọi lỗi: nút
        hỏng cũng im như nút chạy được.

        Giờ: mở thật, và nếu không mở được thì NÓI ra đường dẫn.
        """
        if not RESULT_FILE.exists():
            if self.proxy.rowCount():
                # Bang co ket qua ma file thi khong — bao dung cho, dung noi
                # "chua chay check" trong khi nguoi ta vua chay xong.
                QMessageBox.warning(
                    self, "Không thấy file kết quả",
                    f"Bảng đang có {self.proxy.rowCount()} dòng nhưng không "
                    f"tìm thấy:\n{RESULT_FILE}\n\nFile có thể đã bị xoá hoặc "
                    "di chuyển. Bạn vẫn copy được kết quả từ bảng bằng Cmd+C.")
            else:
                QMessageBox.information(self, "Chưa có kết quả",
                                        "Chạy check xong rồi mở lại nhé.")
            return

        if QDesktopServices.openUrl(QUrl.fromLocalFile(str(RESULT_FILE))):
            self._append_log(f"📂 Đã mở {RESULT_FILE.name}")
            return

        # Khong co app nao nhan mo .csv thi hien no trong Finder cho de tim
        if self._reveal_in_finder(RESULT_FILE):
            self._append_log(f"📂 Đã hiện {RESULT_FILE.name} trong Finder")
            return

        QMessageBox.warning(
            self, "Không mở được file",
            f"File có ở đây nhưng máy không mở được:\n\n{RESULT_FILE}\n\n"
            "Mở tay bằng Numbers hoặc Excel nhé. Kết quả trong bảng thì bạn "
            "vẫn copy được bằng Cmd+C.")

...
TABLE_HEADERS = ("Serial", "Ngày mua", "Trạng thái")


def clean_cell(value):
    """Một ô -> một đoạn text an toàn để dán.

    Tab và xuống dòng trong ô sẽ phá cấu trúc cột lúc dán vào Excel, nên đổi
    hết thành khoảng trắng.
    """
    text = "" if value is None else str(value)
    for xau in ("\r\n", "\r", "\n", "\t"):
        text = text.replace(xau, " ")
    return text.strip()


def rows_to_clipboard_text(rows, with_header=False):
    """Bảng -> văn bản dán thẳng được vào Excel / Numbers / Google Sheets.

    Ngăn bằng Tab chứ không phải dấu phẩy: Excel và Sheets tách cột theo Tab
    ngay khi dán, còn dấu phẩy thì hỏng ngay lần đầu gặp ô có dấu phẩy.

    Một ô lẻ thì trả về đúng nội dung ô đó — copy một serial rồi dán vào ô tìm
    kiếm mà dính thêm dấu xuống dòng là rất khó chịu.
    """
    bang = [[clean_cell(o) for o in dong] for dong in rows]
    if not bang:
        return ""

    if not with_header and len(bang) == 1 and len(bang[0]) == 1:
        return bang[0][0]

    if with_header:
        so_cot = max(len(dong) for dong in bang)
        bang.insert(0, list(TABLE_HEADERS[:so_cot]))

    return "\n".join("\t".join(dong) for dong in bang)

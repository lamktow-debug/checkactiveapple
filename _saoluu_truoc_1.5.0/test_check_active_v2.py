...
class LocalOcrLadderTests(unittest.IsolatedAsyncioTestCase):
    """OCR may chay TRUOC, 2captcha chi do khi OCR truot."""

    def setUp(self):
        import check_active_v2

        self.module = check_active_v2
        self._enabled = check_active_v2.LOCAL_OCR_ENABLED
        self._ocr = check_active_v2.LOCAL_OCR
        self._tries = check_active_v2.LOCAL_OCR_MAX_ATTEMPTS
        check_active_v2.LOCAL_OCR_ENABLED = True

    def tearDown(self):
        self.module.LOCAL_OCR_ENABLED = self._enabled
        self.module.LOCAL_OCR = self._ocr
        self.module.LOCAL_OCR_MAX_ATTEMPTS = self._tries
    ...
    class _OcrGia:
        def __init__(self, cac_ma):
            self.cac_ma = list(cac_ma)
            self.attempts = 0
            self.accepted = 0
            self.da_goi = 0
        ...
        def solve(self, _anh):
            self.da_goi += 1
            ma = self.cac_ma.pop(0) if self.cac_ma else None
            if ma:
                self.attempts += 1
            return ma
    ...
    async def _chay(self, cac_ma_ocr, ket_qua_trang, mua_ma=None):
        ocr = self._OcrGia(cac_ma_ocr)
        self.module.LOCAL_OCR = ocr
        mua = mua_ma or AsyncMock(return_value=CaptchaSolution("MUA1", 99))

        with (
            patch("check_active_v2.open_check_page", new=AsyncMock(return_value=True)),
            patch("check_active_v2.grab_captcha_image", new=AsyncMock(return_value="QUJDRA==")),
            patch("check_active_v2.solve_captcha_task", new=mua),
            patch("check_active_v2.submit_captcha_code", new=AsyncMock()),
            patch("check_active_v2.wait_for_result_payload",
                  new=AsyncMock(side_effect=ket_qua_trang)),
            patch("check_active_v2.has_invalid_serial_input_error",
                  new=AsyncMock(return_value=False)),
            patch("check_active_v2.has_error_message", new=AsyncMock(return_value=True)),
            patch("check_active_v2.report_bad_captcha", new=AsyncMock()),
            patch("check_active_v2.reload_captcha", new=AsyncMock()),
        ):
            ket = await check_serial(self._fake_page(), "C02XG2JRD",
                                     capture_screenshot=False)
        return ket, ocr, mua

    async def test_ocr_may_dung_ngay_thi_khong_dung_toi_2captcha(self):
        ket, ocr, mua = await self._chay(
            ["QW7T"], [{"purchase_date": "28/04/2026"}])
        self.assertEqual(ket, ["C02XG2JRD", "28/04/2026"])
        mua.assert_not_awaited()
        self.assertEqual(ocr.accepted, 1, "phai ghi nhan la Apple chap nhan")

    async def test_ocr_truot_thi_quay_ve_2captcha_ngay(self):
        """CHI thu OCR may mot lan.

        Do that 26/08: thu 2 lan lam 18 serial mat 10 phut, con 2captcha chi 6
        phut. Moi lan truot deu phai mua ma 2captcha SAU DO, nen thu lan hai la
        nhan doi phan lang phi ma khong nhan doi co hoi.
        """
        ket, ocr, mua = await self._chay(
            ["SAI1", "SAI2"],
            [{"purchase_date": None}, {"purchase_date": "28/04/2026"}])
        self.assertEqual(ket, ["C02XG2JRD", "28/04/2026"])
        self.assertEqual(ocr.da_goi, 1, "truot mot lan la di mua ma ngay")
        mua.assert_awaited()
        self.assertEqual(ocr.accepted, 0, "OCR may truot thi khong duoc tinh la dung")

    async def test_ocr_khong_doc_duoc_thi_di_thang_2captcha(self):
        ket, ocr, mua = await self._chay([None], [{"purchase_date": "28/04/2026"}])
        self.assertEqual(ket, ["C02XG2JRD", "28/04/2026"])
        mua.assert_awaited_once()

    async def test_tat_ocr_may_thi_chay_y_nhu_cu(self):
        self.module.LOCAL_OCR_ENABLED = False
        ket, ocr, mua = await self._chay(["QW7T"], [{"purchase_date": "28/04/2026"}])
        self.assertEqual(ket, ["C02XG2JRD", "28/04/2026"])
        self.assertEqual(ocr.da_goi, 0, "tat roi thi khong duoc goi OCR may")
        mua.assert_awaited_once()

    async def test_ma_cua_ocr_may_khong_goi_mang_sang_2captcha(self):
        """Ma khong mua thi khong co gi de doi hoan tien.

        Khong kiem tra "co goi report_bad_captcha khong" — ham do co chot chan
        san. Kiem tra dung dieu quan trong: KHONG co request nao bay sang
        2captcha cho mot ma minh tu doan lay.
        """
        goi_mang = MagicMock(return_value={"errorId": 0})
        ocr = self._OcrGia(["SAI1"])
        self.module.LOCAL_OCR = ocr
        self.module.LOCAL_OCR_MAX_ATTEMPTS = 1

        with (
            patch("check_active_v2.open_check_page", new=AsyncMock(return_value=True)),
            patch("check_active_v2.grab_captcha_image", new=AsyncMock(return_value="QUJDRA==")),
            patch("check_active_v2.solve_captcha_task",
                  new=AsyncMock(return_value=CaptchaSolution("MUA1", 99))),
            patch("check_active_v2.submit_captcha_code", new=AsyncMock()),
            patch("check_active_v2.wait_for_result_payload", new=AsyncMock(
                side_effect=[{"purchase_date": None}, {"purchase_date": "28/04/2026"}])),
            patch("check_active_v2.has_invalid_serial_input_error",
                  new=AsyncMock(return_value=False)),
            patch("check_active_v2.has_error_message", new=AsyncMock(return_value=True)),
            patch("check_active_v2.post_2captcha_json", new=goi_mang),
            patch("check_active_v2.reload_captcha", new=AsyncMock()),
        ):
            await check_serial(self._fake_page(), "C02XG2JRD", capture_screenshot=False)

        cac_url = [goi.args[0] for goi in goi_mang.call_args_list]
        self.assertNotIn(CAPTCHA_2CAPTCHA_REPORT_BAD_URL, cac_url,
                         "da bao sai mot ma khong phai mua cua 2captcha")

...
class ClipboardTextTests(unittest.TestCase):
    """Copy từ bảng kết quả phải dán được thẳng vào Excel."""

    def test_one_cell_has_no_trailing_newline(self):
        """Copy một serial rồi dán vào ô tìm kiếm mà dính \n là rất khó chịu."""
        self.assertEqual(rows_to_clipboard_text([["C02XG2JRD"]]), "C02XG2JRD")

    def test_columns_are_separated_by_tab_not_comma(self):
        """Dấu phẩy hỏng ngay khi một ô có dấu phẩy; Tab thì Excel tự tách cột."""
        text = rows_to_clipboard_text([["C02X", "28/04/2026", "Đã kích hoạt"]])
        self.assertEqual(text, "C02X\t28/04/2026\tĐã kích hoạt")
        self.assertNotIn(",", text)

    def test_many_rows_become_many_lines(self):
        text = rows_to_clipboard_text([["A", "1", "x"], ["B", "2", "y"]])
        self.assertEqual(text.split("\n"), ["A\t1\tx", "B\t2\ty"])

    def test_header_is_added_only_when_asked(self):
        co = rows_to_clipboard_text([["A", "1", "x"]], with_header=True)
        self.assertEqual(co.split("\n")[0], "Serial\tNgày mua\tTrạng thái")
        khong = rows_to_clipboard_text([["A", "1", "x"]])
        self.assertNotIn("Serial\t", khong)

    def test_tabs_and_newlines_inside_a_cell_do_not_break_columns(self):
        """Một ô có \t hoặc \n sẽ đẩy lệch mọi cột sau nó khi dán."""
        text = rows_to_clipboard_text([["A\tB", "dòng1\ndòng2", "x"]])
        self.assertEqual(text, "A B\tdòng1 dòng2\tx")
        self.assertEqual(len(text.split("\n")), 1)
        self.assertEqual(len(text.split("\t")), 3)

    def test_empty_table_gives_empty_text(self):
        """Không có gì thì đừng ghi đè clipboard đang có thứ khác của người ta."""
        self.assertEqual(rows_to_clipboard_text([]), "")

    def test_none_cells_do_not_become_the_word_None(self):
        self.assertEqual(rows_to_clipboard_text([["A", None, "x"]]), "A\t\tx")


class OpenResultsButtonTests(unittest.TestCase):
    """Nút 'Mở file kết quả' phải MỞ file, và phải kêu khi không mở được.

    Bản cũ chỉ gọi `open -R` (chỉ hiện file trong Finder, mà Finder hay không
    nhảy lên trước) kèm check=False (nuốt sạch mọi lỗi) — bấm nút xong không
    thấy gì xảy ra và cũng không có lời báo nào.
    """

    def _source(self):
        import pathlib

        return pathlib.Path("app_qt.py").read_text(encoding="utf-8")
    ...
    def _method(self, name):
        ...
        self.fail(f"khong tim thay ham {name} trong app_qt.py")

    def test_it_actually_opens_the_file(self):
        than = self._method("_open_results")
        self.assertIn("QDesktopServices.openUrl", than,
                      "nut 'Mo file ket qua' phai MO file, khong chi hien trong Finder")

    def test_failures_are_reported_not_swallowed(self):
        than = self._method("_open_results")
        self.assertIn("QMessageBox", than,
                      "khong mo duoc thi phai bao, dung im lang")
        self.assertIn(str("RESULT_FILE"), than)

    def test_no_subprocess_call_in_the_file_silently_ignores_its_exit_code(self):
        """check=False = loi gi cung coi nhu xong. Do la ly do nut hong ma im.

        Quet bang AST chu khong phai tim chuoi: chu 'check=False' con nam
        trong docstring giai thich bug nay.
        """
        import ast
        ...
        self.assertEqual(vi_pham, [], f"subprocess.run(check=False) o dong {vi_pham}")

    def test_reveal_helper_looks_at_the_return_code(self):
        than = self._method("_reveal_in_finder")
        self.assertIn("returncode", than)



class QtImportCompletenessTests(unittest.TestCase):
    """Mọi tên Qt dùng trong app_qt.py phải được import.

    Không cài được PySide6 ở đây nên không chạy thử app_qt.py được; thiếu một
    dòng import thì phải tới lúc bấm đúng cái nút đó app mới nổ NameError.
    Quét tĩnh là cách duy nhất bắt được sớm.
    """
    ...
    def _names(self):
        ...
        tree = ast.parse(pathlib.Path("app_qt.py").read_text(encoding="utf-8"))
        imported = set()
        for node in ast.walk(tree):
        ...
        gan = {n.id for n in ast.walk(tree)
               if isinstance(n, ast.Name) and isinstance(n.ctx, ast.Store)}
        gan.update(n.name for n in ast.walk(tree)
                   if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)))

        dung = {n.id for n in ast.walk(tree)
                if isinstance(n, ast.Name) and isinstance(n.ctx, ast.Load)}
        return imported, gan, dung

    def test_every_qt_name_used_is_imported(self):
        imported, gan, dung = self._names()
        thieu = sorted(x for x in dung
                       if x.startswith("Q") and x not in imported and x not in gan)
        self.assertEqual(thieu, [], f"dung ma chua import: {thieu}")

    def test_the_scan_actually_sees_the_qt_names(self):
        """Doi chieu nguoc: test tren khong duoc xanh vi quet trung."""
        _, _, dung = self._names()
        for ten in ("QMessageBox", "QDesktopServices", "QShortcut", "QKeySequence"):
            self.assertIn(ten, dung, f"{ten} bien mat khoi app_qt.py")

    def test_no_qt_import_is_left_unused(self):
        imported, _, dung = self._names()
        thua = sorted(x for x in imported if x.startswith("Q") and x not in dung)
        self.assertEqual(thua, [], f"import roi khong dung: {thua}")
